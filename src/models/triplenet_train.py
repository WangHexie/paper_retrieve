...
# model = load_file_or_model("modelhardest2_abs_loss_hign_learning_rate5.pk")

model.to(device)
loss_fn = MyOnlineTripletLoss(margin, default_train_config.sample_number, absolute=True)
lr = 1e-3
...
fit(triplet_train_dataset, model, loss_fn, optimizer, scheduler, n_epochs, log_interval,
    name="hardest2_abs_loss_idf", start_epoch=0)
